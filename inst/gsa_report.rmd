...
if (class(result) == "character") {
  result <- readRDS("../test_result.rds")
}
...
```{r pathway_volcano, fig.width=n_2_cols*3, fig.height=n_2_rows*3}
library(ggplot2)
plot_objs <- list()

for (dataset_name in names(result)) {
  plot_obj <- plot_volcano(result, dataset = dataset_name)
  plot_objs[[length(plot_objs) + 1]] <- plot_obj
}

plot_grid(plotlist = plot_objs, ncol = 2)
```
...
for (dataset_name in names(result)) {
  ...
  pathways_up <- pathway_result[pathway_result[, direction_col] == "Up", ]
  pathways_up <- pathways_up[order(pathways_up[, fdr_col], decreasing = F), ]
  pathways_up <- pathways_up[pathways_up[, fdr_col] <= min_p, ]
  
  pathways_down <- pathway_result[pathway_result[, direction_col] == "Down", ]
  ...
  top_pathways <- c(top_pathways, rownames(pathways_up)[1:top_n], rownames(pathways_down)[1:top_n])
}
